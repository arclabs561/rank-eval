...
use rank_eval::trec::{group_runs_by_query, group_qrels_by_query, load_trec_runs, load_qrels};
...
fn main() {
    ...
    for (query_id, query_runs_by_tag) in &runs_by_query {
        ...
        if let Some((_, run_results)) = query_runs_by_tag.iter().next() {
            let ranked: Vec<String> = run_results.iter()
    ...
    println!("✅ Batch evaluation: {} queries", batch_results.query_results.len());
    println!("   Average nDCG@10: {:.4}", batch_results.aggregated.get("ndcg@10").unwrap_or(&0.0));
    
    // Test individual query evaluation
    let query1_runs = &runs_by_query["1"]["run1"];
    let query1_ranked: Vec<String> = query1_runs.iter()
        .map(|(doc_id, _)| doc_id.clone())
        .collect();
