...
mod tests {
    ...
    fn test_full_pipeline_load_validate_evaluate() {
        ...
        assert!(
            runs.len() >= 15,
            "Should load at least 15 runs (got {})",
            runs.len()
        );
        assert!(
            runs.len() <= 20,
            "Should not load excessive runs (got {})",
            runs.len()
        );
        ...
        assert_eq!(
            qrels_by_query.len(),
            3,
            "Should have exactly 3 queries in qrels"
        );
        ...
        assert!(
            query1_runs.contains_key("bm25"),
            "Query 1 should have bm25 run"
        );
    ...
    fn test_full_pipeline_statistics() {
        ...
        assert!(
            stats.runs.total_entries >= 15 && stats.runs.total_entries <= 20,
            "Should have approximately 15 run entries"
        );
        assert_eq!(stats.runs.unique_queries, 3, "Should have 3 unique queries");
        assert_eq!(
            stats.runs.unique_run_tags, 3,
            "Should have 3 run tags (bm25, dense, hybrid)"
        );
        assert_eq!(
            stats.qrels.total_entries, 8,
            "Should have exactly 8 qrel entries"
        );
        assert_eq!(
            stats.overlap.queries_in_both, 3,
            "Should have 3 queries in both"
        );
        assert!(
            stats.quality.queries_with_multiple_runs > 0,
            "Should have queries with multiple runs"
        );
        assert!(
            stats.quality.fusion_readiness_ratio > 0.0,
            "Should have positive fusion readiness"
        );
    }
    ...
    fn test_multi_query_evaluation() {
        ...
        for (query_id, query_qrels) in &qrels_by_query {
            if let Some(query_runs) = runs_by_query.get(query_id) {
                if let Some(bm25_run) = query_runs.get("bm25") {
                    let ranked_ids: Vec<&str> =
                        bm25_run.iter().map(|(id, _)| id.as_str()).collect();
        ...
        assert!(
            all_ndcgs.iter().all(|&n| n >= 0.0 && n <= 1.0),
            "All nDCG values should be valid"
        );
    }
    ...
    fn test_dataset_loader_functions() {
        ...
        assert!(
            runs.len() >= 15,
            "MS MARCO loader should load at least the expected runs"
        );
        assert!(
            runs.len() <= 20,
            "MS MARCO loader should not load excessive runs"
        );
        ...
        assert_eq!(
            qrels.len(),
            8,
            "MS MARCO qrels loader should load all qrels"
        );
        ...
        assert!(
            beir_runs.len() >= 15 && beir_runs.len() <= 20,
            "BEIR loader should load approximately 15 runs (got {})",
            beir_runs.len()
        );
        ...
        assert_eq!(
            beir_qrels.len(),
            8,
            "BEIR qrels loader should load all qrels"
        );
    }
    ...
    fn test_validation_comprehensive() {
        ...
        assert!(
            validation.statistics.runs_count >= 15 && validation.statistics.runs_count <= 20,
            "Should have approximately 15 runs"
        );
        assert_eq!(
            validation.statistics.qrels_count, 8,
            "Should have exactly 8 qrels"
        );
        assert_eq!(
            validation.statistics.queries_in_both, 3,
            "Should have 3 queries in both"
        );
    }
    ...
    fn test_statistics_comprehensive() {
        ...
        assert_eq!(
            stats.runs.total_entries, 17,
            "Should have exactly 17 run entries (6+2+9)"
        );
        assert_eq!(stats.runs.unique_queries, 3, "Should have 3 unique queries");
        assert!(
            stats.runs.unique_documents >= 8,
            "Should have at least 8 unique documents"
        );
    ...
    fn test_score_distribution_properties() {
        ...
        assert!(
            dist.percentiles.p25 <= dist.percentiles.p50,
            "P25 should be <= P50"
        );
        assert!(
            dist.percentiles.p50 <= dist.percentiles.p75,
            "P50 should be <= P75"
        );
        assert!(
            dist.percentiles.p75 <= dist.percentiles.p90,
            "P75 should be <= P90"
        );
        assert!(
            dist.percentiles.p90 <= dist.percentiles.p95,
            "P90 should be <= P95"
        );
        assert!(
            dist.percentiles.p95 <= dist.percentiles.p99,
            "P95 should be <= P99"
        );
