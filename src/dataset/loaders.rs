...
use crate::trec::{Qrel, TrecRun, load_trec_runs, load_qrels};
...
pub fn load_trec_qrels_from_dir(qrels_dir: impl AsRef<Path>) -> Result<Vec<Qrel>> {
    ...
    anyhow::bail!("No qrels file found in {:?}. Tried: {:?}", qrels_dir.as_ref(), possible_names);
}
...
pub fn list_datasets(datasets_dir: impl AsRef<Path>) -> Result<Vec<String>> {
    let dir = std::fs::read_dir(datasets_dir.as_ref())
        .with_context(|| format!("Failed to read datasets directory: {:?}", datasets_dir.as_ref()))?;
...
pub fn validate_dataset_dir(dataset_dir: impl AsRef<Path>) -> Result<bool> {
    ...
    let has_qrels = possible_qrels.iter()
...
pub fn get_dataset_stats(runs: &[TrecRun], qrels: &[Qrel]) -> DatasetStats {
    ...
    let queries_with_qrels: std::collections::HashSet<_> = qrels.iter().map(|q| &q.query_id).collect();
